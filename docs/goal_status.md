...
| 후면 영상 기준 phase detection 고도화 | `service/server/analysis/phase.py` | 구현 완료, 실영상 검증 전 |
| pelvis/torso/body-scale 기반 2D 정규화 반영 | `service/server/analysis/normalization.py`, `docs/scoring_policy.md` | 구현 완료, 실영상 검증 전 |
...
| 테스트 실행 금지 | exp08 검증 테스트는 미실행. 사용자 요청에 따른 로컬 웹 서버 실행은 별도 | 준수 |
...
| 후면 영상 기준 phase detection 고도화 | `service/server/analysis/phase.py`, `docs/scoring_policy.md` | keypoints 기반 v1 휴리스틱 구현됨. 실영상 검증은 미실행 |
| pelvis/torso/body-scale 기반 2D 공간 정규화 반영 | `service/server/analysis/normalization.py`, `docs/scoring_policy.md` | 분석용 body-frame 좌표 생성 구현 및 문서화됨 |
...
| 테스트 실행은 요청 전까지 하지 않음 | exp08 검증 테스트는 미실행. 웹사이트 확인 요청에 따른 로컬 웹 서버 실행은 별도 | 준수 |
...
| 3 | 후면 영상 기준 phase detection을 keypoints 기반으로 고도화 | `service/server/analysis/phase.py`, `docs/scoring_policy.md` | 구현 완료, 실영상 검증 전 |
| 4 | pelvis/torso/body-scale 기반 2D 공간 정규화를 유사도 계산에 반영 | `service/server/analysis/normalization.py`, `service/server/analysis/similarity.py` | 구현 완료, 실영상 검증 전 |
